...
- [ ] All twelve current local routes exist and open without errors.
...
- [ ] The primary demo path clearly shows rough context -> governed spec -> decision/evidence gate -> handoff preview without adding a route beyond the GOAL-021 twelve-route surface.
...
- [ ] `docs/product/STATIC_MVP_SCREEN_MAP.md` maps all twelve current local routes to the locked journey/object model without adding routes.
...
- [ ] Object visibility is verified across the current twelve local routes: Project, Context Nodes, Selected Context, Spec Draft, Review Run, Findings, Decisions, Evidence, Artifact Reference, Work Packet, Handoff Packet and Validation Results.
- [ ] Scope-creep checks confirm no routes beyond GOAL-021, dependencies, backend/API/auth/database/deployment, runtime workflow execution, real orchestration, external connectors, MCP, secrets, real export, filesystem API or app-driven repo write behavior.
