...
6. **Workbench interaction** - Selecting an object from Object Outline or the board highlights it without forcing the legacy right inspector open. The Workbench default reads as a SpecGraph scenario-lens editor and visually prioritizes three macro layers: Context, SpecGraph and Handoff Gates. The full depth remains available in Object Outline, Inspector, Trace, focus lenses, `guided_flow`, `lineage_impact_map`, `control_plane`, and canvas drill-down views. The Workbench still includes a whole-board spatial canvas with fixed mock positions, visual zones, zoom in/out, fit, reset, mouse-wheel zoom and drag-to-pan. The object editor panel shows Inspector, Edit Fields, Copilot, Evidence and Trace tabs for the selected object. The focus lens selector exposes `guided_flow`, `lineage_impact_map`, `control_plane` and `code_review_lens` without resetting selected object, viewport, view mode, right panel tab or outline expansion. The readiness queue groups missing evidence, unlocked decisions, review blockers, validation blockers and handoff blockers. The context dock updates from the selected object plus Selected Context and protected exclusions, without typed chat or real AI calls. `lineage_impact_map` remains available as the requirement drilldown where a requirement expands into architecture children while sibling requirements remain visible. `control_plane` remains available as the optional ordered grid of task nodes `01` to `08`. The object-type popover lists cockpit object types, not executable workflow nodes. The Selected Context popover lists included context and protected exclusions. Adding, removing, and clearing Selected Context update local browser state only.
...
- **Focus lenses** – `guided_flow`, `lineage_impact_map`, `control_plane` and `code_review_lens` visibly change highlights without resetting selected object, board viewport, view mode, right panel tab or outline expansion.
- **Code-object lens** – Choosing `code_review_lens` or selecting the `Inspector & Sidebar Rails` Workbench object shows a static bounded technical lens in the existing object editor panel. Facts, relations, findings with evidence chips, and AI annotations are visually separated. Every finding has one or more evidence refs, and the panel does not add a route, dependency, backend, external action, source intake, or app-side write behavior.
...
- **SpecGraph-first read** – On Workbench, the light dotted spatial board reads as the `guided_flow` lens over SpecGraph objects, with palette, context dock and inspector surfaces supporting scenario review rather than dominating it.
...
- Workbench keeps the spatial board default, reduces canvas toolbar chrome, uses popovers for object types and Selected Context, renders a context dock for the selected object, preserves `lineage_impact_map` as an optional drilldown with sibling requirements visible during expansion, preserves `control_plane` as an optional sequence view, and links to Preview readiness.
- GOAL-015 accepts the Workbench spatial canvas only when the board renders all current hierarchy objects by default, zoom in/out, fit, reset, mouse-wheel zoom and drag-to-pan work in local browser state, the context dock updates on selection, no browser storage persistence APIs are introduced, no remote HTTP/HTTPS requests occur, and `lineage_impact_map` / `control_plane` remain available as secondary review modes.
