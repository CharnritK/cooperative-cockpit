...
6. **Workbench interaction** - Selecting a Context Node highlights it without forcing the right inspector open. The inspector opens from the top-bar inspector icon and updates to the selected node while open. The Workbench defaults to a whole-board spatial canvas that renders every current hierarchy object with fixed mock positions, visual zones, zoom in/out, fit, reset, mouse-wheel zoom and drag-to-pan. The context dock updates from the selected object plus Selected Context and protected exclusions, without typed chat or real AI calls. Mixed Map remains available as a secondary requirement drilldown where a requirement expands into architecture children while sibling requirements remain visible. Flat Flow remains available as an optional ordered grid of task nodes `01` to `08`. The Workbench does not render the redundant primary workflow strip or a persistent support sidebar; orientation comes from the readiness rail, object canvas, canvas toolbar and context dock. The object-type popover lists cockpit object types, not executable workflow nodes. The Selected Context popover lists included context and protected exclusions. Adding, removing, and clearing Selected Context update local browser state only.
