...
6. **Workbench interaction** – Selecting a node highlights it and opens the right inspector. Visual connectors render between nodes from `mockData.workflowEdges`. The primary demo strip reads “rough context -> governed spec -> decision/evidence gate -> handoff preview”. The node palette lists OpenClaw-safe workflow node types. Clicking “Add selected” adds the node to the context basket. The context basket lists included context and protected exclusions. Removing items and clearing the basket update the view.
...
11. **Trace & Evidence interaction** – The page displays a placeholder graph, an evidence table for source context, locked decision, validation readiness and handoff placeholder, plus trace links. A warning appears if required evidence or spec fields lack trace evidence. No file-producing handoff control is active because the trace is incomplete.
