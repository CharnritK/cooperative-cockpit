...
This document supports the root `OPERATING_WORKFLOW.md`, which is the authoritative agent governance workflow. If this file conflicts with root `OPERATING_WORKFLOW.md`, the root workflow wins.
...
5. **Source control** – Stage, commit, push, or open a pull request only when the task packet or Point explicitly authorizes that action. Never merge or deploy without approval.
...
7. **Handoff or merge** – Once approved, deliver the handoff package or request merge according to the approval policy.
